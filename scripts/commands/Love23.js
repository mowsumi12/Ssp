...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'cache/canvas', 'crush6312.png');
  if (!existsSync(dirMaterial + "canvas")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.imgur.com/ZuLqfnh.jpeg", path); 
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/crush6312.png");
  ...
  batgiam_img.composite(circleOne.resize(133, 133), 43, 48).composite(circleTwo.resize(133, 133), 305, 46);
